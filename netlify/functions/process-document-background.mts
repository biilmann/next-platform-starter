...
import { processNotionImages, processMarkdownImages } from '../lib/image-utils.mjs';
...
async function convertToMetadataAndMarkdown(openai: any, type: DocumentType, doc: string, docId: string) {
  // Get current date for proper publishDate
...
export default async function handler(req: Request) {
  ...
  try {
    // Mark as processing with step details
    ...
    // Process images before AI conversion (only for Notion documents)
    let imageMapping = {};
    if (type === 'notion') {
      await drafts.set(`${id}-status`, JSON.stringify({ 
        status: 'processing', 
      ...
      try {
        ...
        if (docData.children && docData.children.results) {
          // Extract title for image naming
          ...
          // Download images while URLs are valid
    ...
    // Process metadata and markdown in single API call
    ...
    // Process markdown to replace image URLs with local paths
    const finalMarkdown = Object.keys(imageMapping).length > 0 
      ? processMarkdownImages(result.markdown, imageMapping)
      : result.markdown;

    // Store the processed content with local image references
